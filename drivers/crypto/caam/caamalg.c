...
static int des_ablkcipher_setkey(struct crypto_ablkcipher *skcipher,
				 const u8 *key, unsigned int keylen)
{
	u32 tmp[DES3_EDE_EXPKEY_WORDS];
	struct crypto_tfm *tfm = crypto_ablkcipher_tfm(skcipher);
	...
	if (!des_ekey(tmp, key) && (crypto_ablkcipher_get_flags(skcipher) &
	    CRYPTO_TFM_REQ_WEAK_KEY)) {
		crypto_ablkcipher_set_flags(skcipher,
					    CRYPTO_TFM_RES_WEAK_KEY);
		return -EINVAL;
	}

	return ablkcipher_setkey(skcipher, key, keylen);
}
				   ...
				   void *context)
{
	 ...
	 */
	if (ivsize) {
		memcpy(req->info, (u8 *)edesc->sec4_sg + edesc->sec4_sg_bytes,
		       ivsize);
						       ...
						       *req, int desc_bytes)
{
	 ...
	 */
	sec4_sg_ents += (mapped_dst_nents > 1 || ivsize) ?
			!!ivsize + mapped_dst_nents : 0;
	...
	edesc->sec4_sg = (struct sec4_sg_entry *)((u8 *)edesc->hw_desc +
						  desc_bytes);
	edesc->iv_dir = DMA_BIDIRECTIONAL;
	...
	if (dst_sg_idx)
		sg_to_sec4_sg(req->src, req->nbytes, edesc->sec4_sg + !!ivsize,
			      0);

	if (req->src != req->dst && (ivsize || mapped_dst_nents > 1))
		sg_to_sec4_sg(req->dst, req->nbytes, edesc->sec4_sg +
			      dst_sg_idx, 0);
	...
	if (sec4_sg_bytes) {
		...
		if (dma_mapping_error(jrdev, edesc->sec4_sg_dma)) {
			dev_err(jrdev, "unable to map S/G table\n");
			caam_unmap(jrdev, req->src, req->dst, src_nents,
				   dst_nents, iv_dma, ivsize, DMA_TO_DEVICE, 0,
				   0);
			kfree(edesc);
			return ERR_PTR(-ENOMEM);
		}
